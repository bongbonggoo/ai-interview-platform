...
사용자가 **기관을 고르면 그 기관 인재상에 맞는 면접 질문을 만들어 주고**, 답변을 쓰면
그 인재상을 기준으로 **어느 역량이 어디서 드러났고 무엇이 약한지**를 원문을 짚어가며 피드백한다.
관리자가 질문이나 루브릭을 미리 입력해두는 제품이 아니다 — 질문은 AI가 만들고,
기준(인재상)은 시드·리서치로 준비돼 있어야 한다.

## 절대 지켜야 할 원칙 (코드 리뷰 시 이 기준으로 판단할 것)

0. **모범답안을 만들지도, 저장하지도, 보여주지도 않는다.** 채점 기준은 "이 역량이 어떻게
   드러났는가"를 관찰하는 행동기준(1/3/5)이지 예시 답변이 아니다. 프롬프트에 "이렇게 쓰면
   좋다"류를 넣거나 지원자 문장을 고쳐 쓰게 하는 변경은 이 원칙 위반이다. ERD의
   `golden_answer` 테이블도 이 이유로 만들지 않는다.
...
2. **역량(`CompetencyLibrary`)은 회사 무관 재사용 자산이고, 회사 인재상은 출처가 있어야 한다.**
   `CompetencyEvidence.sourceUrl`이 필수인 이유 — 인재상 원문을 확인하지 못한 기관은
   역량을 추측으로 채우지 말고 비워두고(`competencyConfirmed=false`) 공통 기준으로만 평가한다. 회사별로 새 역량 이름을
